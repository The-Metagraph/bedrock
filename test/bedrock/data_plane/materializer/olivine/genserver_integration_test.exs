defmodule Bedrock.DataPlane.Materializer.Olivine.GenServerIntegrationTest do
  @moduledoc """
  ...
  describe "GenServer Lifecycle Integration" do
    ...
    test "GenServer survives under supervision", %{tmp_dir: tmp_dir} do
      ...
      assert {:ok, :materializer} = GenServer.call(new_worker_pid, {:info, :kind}, @timeout)
    ...
    test "GenServer handles concurrent init requests", %{tmp_dir: tmp_dir} do
      ...
      assert {:ok, :materializer} = GenServer.call(successful_pid, {:info, :kind}, @timeout)
    ...
    test "recovery lock/unlock operations via GenServer", %{tmp_dir: tmp_dir} do
      ...
      case result do
        {:ok, returned_pid, recovery_info} ->
          ...
          # Should contain required recovery info with :materializer kind
          if is_list(recovery_info) do
            assert [kind: :materializer, durable_version: _, oldest_durable_version: _] =
                     Keyword.take(recovery_info, [:kind, :durable_version, :oldest_durable_version])
          else
            assert %{kind: :materializer, durable_version: _, oldest_durable_version: _} = recovery_info
          end
  ...
  describe "GenServer Message Handling and State Management" do
    ...
    test "GenServer graceful shutdown preserves data", %{tmp_dir: tmp_dir} do
      ...
      assert {:ok, :materializer} = GenServer.call(new_pid, {:info, :kind}, @timeout)
  ...
  describe "Integration with Bedrock Storage Interface" do
    @tag :tmp_dir
    test "Materializer.get/4 function works with Olivine GenServer", %{tmp_dir: tmp_dir} do
      worker_id = random_id()
      ...
      try do
        result = Materializer.get(pid, "test:key", v0, timeout: 1_000)
      ...
      result2 = Materializer.get(pid, "another:key", v0)
    ...
    @tag :tmp_dir
    test "Materializer.get_range/5 function works with Olivine GenServer", %{tmp_dir: tmp_dir} do
      worker_id = random_id()
      ...
      try do
        result = Materializer.get_range(pid, "start:key", "end:key", v0, timeout: 1_000)
